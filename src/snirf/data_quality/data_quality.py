...
filepath = '/home/darshana/Desktop/FSM_v2-dashboard/FSM-V2-Dashboard/src/uploads/PHANTOM_fsm_data_20241126_151441.xlsx'
...
# Assuming snr_dict and group_snr dictionaries are already available
...
# 3. Execute the plotting functions
# Plot the histogram
plot_snr_histogram(snr_dict)

# Plot the 6 indicators for the average SNR of each group
plot_snr_gauge(group_snr)
...
# Calculate the NEP for each dark detector as you already did
nep_led_A_dark_DET1 = np.std(data['LED_A_DARK_DET1'])
nep_led_A_dark_DET2 = np.std(data['LED_A_DARK_DET2'])
nep_led_A_dark_DET3 = np.std(data['LED_A_DARK_DET3'])

nep_led_B_dark_DET1 = np.std(data['LED_B_DARK_DET1'])
nep_led_B_dark_DET2 = np.std(data['LED_B_DARK_DET2'])
nep_led_B_dark_DET3 = np.std(data['LED_B_DARK_DET3'])

# Collect NEP values and corresponding group names
nep_values = [
    nep_led_A_dark_DET1, nep_led_A_dark_DET2, nep_led_A_dark_DET3,
    nep_led_B_dark_DET1, nep_led_B_dark_DET2, nep_led_B_dark_DET3
]
...
# Create a bar plot for the NEP values
fig = go.Figure()

fig.add_trace(go.Bar(
    x=group_names,
    y=nep_values,
    text=nep_values,  # Display the NEP values on top of each bar
    textposition='outside',  # Place the NEP values outside the bars
    marker=dict(color='royalblue'),  # Choose a color for the bars
))
