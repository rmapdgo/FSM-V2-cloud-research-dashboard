...
def update_tabs_and_plots(view_intensity_n_clicks, check_data_quality_n_clicks, calculate_concentrations_n_clicks,download_concentrations_n_clicks, groupA1_on, groupA2_on, groupB1_on, groupB2_on, select_all_on, selected_spectra, filepath):
    ...
    # Handling for "Download Concentrations Excel File" button
        ...
        # Specify the path to the file you want to provide for download
        file_path = 'src/concentration_data/concentrations.xlsx'

        if os.path.exists(file_path):  # Ensure the file exists
            # Read the file and send it to the browser as download
            with open(file_path, 'rb') as f:
                file_data = f.read()

            # Trigger the download using dcc.send_bytes, specifying the filename
            return dcc.send_bytes(file_data, filename='concentrations.xlsx')
